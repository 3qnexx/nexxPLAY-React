...
var NexxWidget = /*#__PURE__*/function (_NexxBase) {
  _inherits(NexxWidget, _NexxBase);

  var _super = _createSuper(NexxWidget);

  function NexxWidget(props) {
    var _this;

    _classCallCheck(this, NexxWidget);

    _this = _super.call(this, props, ['launchMode', 'userHash'], 'widget');

    if (_this.props.widgetHash) {
      _this.config.widgetHash = _this.props.widgetHash;
    } else {
      _this.config.widgetHash = "";
    }

    if (_this.props.secret) {
      _this.config.secret = _this.props.secret;
    }

    window.removeEventListener("message", _this.onPostMessage);

    if (_this.props.onWidgetClick) {
